...
action :before_migrate do
  ...
  if new_resource.bundler
    ...
    execute "#{bundle_command} install --path=vendor/bundle --binstubs=#{node[:application_geocms][:bundler_binstubs_path]} #{bundler_deployment ? "--deployment " : ""}--without #{common_groups}" do
